  ...
  useEffect(() => {
    if (listing && isOpen) {
      setFormData({
        ...
        latitude: listing.latitude || 36.7538,
        longitude: listing.longitude || 3.0588,
        speciality: listing.speciality || '',
      });
    }
  }, [listing, isOpen]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4 sm:p-6 transition-all">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        <div className="flex justify-between items-center p-6 border-b border-slate-100 bg-slate-50/50">
          <div>
            <h2 className="text-2xl font-extrabold text-slate-900">{t('update_listing')}</h2>
            <p className="text-sm text-slate-500 font-light">{t('modify_listing_desc')}</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className='flex flex-col md:flex-row gap-8'>
            <div className='flex flex-col gap-4 flex-1'>
              ...
              <div className='flex flex-col gap-2 mt-2'>
                ...
                <p className='text-[10px] text-slate-400'>Click on the map to update the location of the workspace.</p>
              </div>
