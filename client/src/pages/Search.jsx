...
export default function Search() {
  ...
  useEffect(() => {
    ...
    const fetchListings = async () => {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/listing/get?${searchQuery}`, { credentials: 'include' });
      ...
      <div className='flex flex-col md:flex-row min-h-screen pt-20 bg-slate-50'>
        ...
        <div className='w-full md:w-[340px] bg-white border-b md:border-b-0 md:border-r border-slate-100 p-6 sm:p-8 flex-shrink-0 shadow-[4px_0_24px_rgba(0,0,0,0.02)] z-10 h-auto md:min-h-screen'>
          <div className='flex items-center gap-2 mb-8'>
            ...
            <h2 className='text-xl font-extrabold text-slate-900'>{t('filter_search')}</h2>
          </div>
          ...
          <form onSubmit={handleSubmit} className='flex flex-col gap-6'>
            ...
            <div className='flex flex-col gap-2'>
              <label className='text-sm font-semibold text-slate-700'>{t('search_keyword')}</label>
            ...
            <div className='flex flex-col gap-3 pt-4 border-t border-slate-100'>
              <label className='text-sm font-semibold text-slate-700'>{t('listing_category')}</label>
              <div className='flex flex-wrap gap-3'>
                {[
                  { id: 'all_cat', label: t('categories_all'), value: 'all' },
                  { id: 'property', label: t('workspaces'), value: 'property' },
                  { id: 'service', label: t('services'), value: 'service' }
                ].map((cat) => (
            ...
            {sidebardata.category !== 'service' && (
              <div className='flex flex-col gap-3 pt-4 border-t border-slate-100'>
                <label className='text-sm font-semibold text-slate-700'>{t('property_type')}</label>
                <div className='flex flex-wrap gap-3'>
                  {['all', 'rent', 'sale'].map((type) => (
                    <label key={type} className={`cursor-pointer flex items-center justify-center px-4 py-2 rounded-lg border text-xs font-semibold transition-all ${sidebardata.type === type ? 'bg-indigo-50 border-indigo-200 text-indigo-700' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
                      ...
                      {type === 'all' ? t('rent_sale') : type === 'rent' ? t('rent') : t('sale')}
                    </label>
                  ))}
                </div>
              </div>
            ...
            <div className='flex flex-col gap-3 pt-4 border-t border-slate-100'>
              <label className='text-sm font-semibold text-slate-700'>{t('features_offers')}</label>
              <div className='flex flex-col gap-3'>
                {[
                  { id: 'offer', label: t('special_offers_only'), alwaysShow: true },
                  { id: 'parking', label: t('includes_parking'), propertyOnly: true },
                  { id: 'furnished', label: t('fully_furnished'), propertyOnly: true }
                ].map((feature) => {
            ...
            <div className='flex flex-col gap-2 pt-4 border-t border-slate-100'>
              <label className='text-sm font-semibold text-slate-700'>{t('sort_results')}</label>
              <select
                onChange={handleChange}
              ...
              >
                <option value='createdAt_desc'>{t('sort_latest')}</option>
                <option value='createdAt_asc'>{t('sort_oldest')}</option>
                <option value='regularPrice_desc'>{t('sort_price_high')}</option>
                <option value='regularPrice_asc'>{t('sort_price_low')}</option>
                <option value='averageRating_desc'>{t('top_rated')}</option>
              </select>
            </div>
        ...
        <div className='flex-1 p-6 md:p-10 flex flex-col'>
          <div className='mb-8'>
            ...
            <p className='text-slate-500 font-light'>{t('discover_perfect_env')}</p>
          </div>
          ...
          <div className='flex-1 flex flex-col'>
            {loading ? (
              <div className='flex-1 flex flex-col items-center justify-center'>
                ...
                <p className='text-slate-500 font-medium'>{t('searching_spaces')}</p>
              </div>
            ) : listings.length === 0 ? (
              <div className='flex-1 flex flex-col items-center justify-center text-center'>
                ...
                <h3 className='text-xl font-bold text-slate-800 mb-2'>{t('no_workspaces_found')}</h3>
                <p className='text-slate-500 max-w-sm'>
                  {t('no_listings_matching')}
                </p>
